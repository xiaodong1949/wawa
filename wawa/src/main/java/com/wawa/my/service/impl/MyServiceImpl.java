...
public class MyServiceImpl extends BasicServiceImpl implements MyService {
    ...
    public List<Resource> getMyResource(int userId, int resourceType) throws Exception {
        String sql = "select * from  t_resource where userid=:userid and resourcetype=:resourcetype order by createtime desc";
        Map<String, Object> paramMap = new HashMap<String, Object>();
        paramMap.put("userid", userId);
        paramMap.put("resourcetype", resourceType);
        List<Resource> listResource = myDao.getObjectList(sql, paramMap, Resource.class);
        return listResource;
    }
